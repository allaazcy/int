...
RUN echo 'Muxiin' | vncpasswd -f > $HOME/.vnc/passwd
...
RUN echo 'whoami ' >>/Muxiin.sh

RUN echo 'cd ' >>/Muxiin.sh

RUN echo "su -l -c 'vncserver :2000 -geometry 1360x768' "  >>/Muxiin.sh

RUN echo 'cd /noVNC-1.2.0' >>/Muxiin.sh

RUN echo './utils/launch.sh  --vnc localhost:7900 --listen 8900 ' >>/Muxiin.sh

RUN chmod 755 /Muxiin.sh
